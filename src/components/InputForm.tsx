...
function InputForm(){
  ...
  const [mode, setMode] = useState<'2' | '3' | 'congruence'>('2'); 
...
const handleSolve = () => {
  if(mode === '2') {
    ...
    const numA = Number(a), numB = Number(b), numM = Number(m );
...
return (
  ...
  {result2 && (
    <div>
      ...
      {result2.particular && (
        <p> Particular solution: (x, y) = ({result2.particular.x}, {result2.particular.y})</p>
        )}
      ...
      {result2.step && result2.particular && (
          ...
          <p>
            {formatVectorSolution2D( result2.particular.x, result2.particular.y, result2.step.dx, result2.step.dy)}</p>
         </>
      )}
    </div>
  )}
  ...
  {result3 && (
   <div>
    ...
    {/* General solution using parameters t and s */}
    {result3.step && result3.particular && (
      <>
        ...
        <p>General solution (vector form):</p>
        <p>{formatVectorSolution3D(result3.particular.x, result3.particular.y, result3.particular.z, result3.step.dx, result3.step.dy, result3.step.dz)}</p>
      </>
    )}
   </div>
  )}
